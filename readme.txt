...
can select if the post is either an appetizer, entree, or dessert. The user can also select which chefusername submitted the
custom post type. Additionally the user can select any number of positive posts.

=Custom Post Type=
Subscriber gallery.
If they specify a recipe link, then a button will be shown to allow the user to see the recipe.
